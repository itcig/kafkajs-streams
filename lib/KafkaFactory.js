...
const KafkaJsClient = require("./client/KafkaJsClient.js");
...
class KafkaFactory {
    ...
    getKafkaClient(topic) {
        ...
        if (this.config.noptions && typeof this.config.noptions === "object") {
            if (this.config.client && this.config.client === 'kafkajs') {
                debug("creating new kafkajs client");
                return new KafkaJsClient(topic, this.config, this.batchOptions);
            }  else {
